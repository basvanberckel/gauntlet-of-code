...
solve:
    xor r10, r10 ; zero a "counter"
    call atoi_read ; rax = atoi(readline())
    mov r11, rax ; prev = rax
.top:
    call atoi_read ; rax = atoi(readline())
    test rax, rax ; if no more lines
    jz .break ; break
    cmp rax, r11 ; rax > prev
    jle .skip_inc ; if rax > prev {
    inc r10 ; ++counter
.skip_inc: ; }
    mov r11, rax ; prev = rax
    jmp .top
.break:
    mov rax, r10 ; return counter
    ret
